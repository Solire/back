...
(function($, window, document) {
	...
	_oExt.oSort = {
		 ...
		 * french date sorting
		 */
		"datefr-asc": function ( a, b )
		{
			var tabx = a.substr(0, 10).split('/');
			// var x = Date.parse( tabx[1]+'/'+tabx[0]+'/'+tabx[2] );
			var x = parseInt(tabx[2] + tabx[1] + tabx[0] + a.substr(11).replace(':', ''));
			
			var taby = b.substr(0, 10).split('/');
			// var y = Date.parse( taby[1]+'/'+taby[0]+'/'+taby[2] );
			var y = parseInt(taby[2] + taby[1] + taby[0] + b.substr(11).replace(':', ''));
			
			if ( isNaN(x) || x==="" )
			{
				x = 0; //Date.parse( "01/01/1970 00:00:00" );
			}

			if ( isNaN(y) || y==="" )
			{
				y =	0; //Date.parse( "01/01/1970 00:00:00" );
			}
			
			return x - y;
		},

		"datefr-desc": function ( a, b )
		{
			var tabx = a.substr(0, 10).split('/');
			// var x = Date.parse( tabx[1]+'/'+tabx[0]+'/'+tabx[2] );
			var x = parseInt(tabx[2] + tabx[1] + tabx[0] + a.substr(11).replace(':', ''));
